...
// (function($) { 
	$(function(){
		...
		(function(o){ //EventsCalendar
			o = $(o);
			if (!o.size()) return;
			if (!EventsCalendarLang) EventsCalendarLang = {'months': 'январь,февраль,март,апрель,май,июнь,июль,август,сентябрь,октябрь,ноябрь,декабрь'.split(",")};
			var today = new Date();
			...
			var monthDraw = function(m, p){ // m - месяц в теущем году или смещение (+-) относительного текущего месяца
				...
				m = mDate.getMonth();
				y = mDate.getFullYear();
				$(".curmonth .name", o).text(EventsCalendarLang["months"][m] + " " + y);
